	...
	if (message.Group_ID !== null) {
		...
		setPlayers((prevPlayers) => {
			...
			const activeUserIds = new Set(
				message.Members.map((member) => member.User_ID),
			);
			...
			message.Members.forEach((member) => {
				const existingPlayerIndex = filteredPlayers.findIndex(
					(player) => player.user_id === member.User_ID,
				);
				...
				if (existingPlayerIndex !== -1) {
					...
					filteredPlayers[existingPlayerIndex] = {
						...
						credits:
							member.Credits !== null
								? member.Credits
								: filteredPlayers[existingPlayerIndex].credits,
					};
				} else {
					//add new player
			...
			const currentUser = message.Members.find(
				(member) => member.Credits !== null,
			);
			if (currentUser) {
