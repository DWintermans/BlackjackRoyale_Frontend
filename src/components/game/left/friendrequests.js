...
export default function FriendRequests({ onGoBack }) {
    ...
    const handleRequest = async (friendid, status) => {
        try {
            ...
            if (status == "accepted") {
                const request = friendRequests.find(request => request.user_id === friendid);
                                    ...
                                    {request.can_answer ? (
                                        request.status === undefined || request.status === null ? (
                                            <div className="space-x-2 flex">
                                                <div
                                                    onClick={() => handleRequest(request.user_id, "accepted")}
                                                    className="bg-[#a8dba8] text-black border border-black py-1 px-3 rounded-lg font-bold hover:bg-[#6c9a8b] cursor-pointer"
                                                >
                                                    <svg
                                                        xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" width="24" height="24" strokeWidth="2">
                                                        <path d="M12 12m-9 0a9 9 0 1 0 18 0a9 9 0 1 0 -18 0"></path>
                                                        <path d="M9 12l2 2l4 -4"></path>
                                                    </svg>
                                                </div>
                                                <div
                                                    onClick={() => handleRequest(request.user_id, "rejected")}
